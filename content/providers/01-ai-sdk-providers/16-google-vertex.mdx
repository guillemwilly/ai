...
Chirp authenticates with the same Google Cloud credentials as the other Vertex
models but calls the regional Cloud Speech-to-Text endpoints. Set
...
and `asia-southeast1`; `chirp_3` in the `us` and `eu` multi-regions. There is no
global endpoint, and these regions differ from Vertex AI regions.

The synchronous API transcribes audio under one minute. The spoken language is
auto-detected by default; pass `languageCodes` to restrict it.
...
| --------- | ------------------- | ----------------------- |
| `chirp_2` | <Check size={18} /> | <Check size={18} />     |
| `chirp_3` | <Check size={18} /> | <Check size={18} />     |
