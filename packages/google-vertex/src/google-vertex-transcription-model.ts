...
export class GoogleVertexTranscriptionModel implements TranscriptionModelV4 {
  ...
  async doGenerate(
    ...
    // Chirp uses regional Speech-to-Text endpoints; there is no global host.
    const url =
      `https://${region}-speech.googleapis.com/v2/projects/` +
    ...
    const segments = results.flatMap(
      result =>
        result.alternatives?.[0]?.words?.map(word => ({
          text: word.word ?? '',
          startSecond: parseDurationSeconds(word.startOffset) ?? 0,
          endSecond: parseDurationSeconds(word.endOffset) ?? 0,
        })) ?? [],
    );
    // Chirp returns BCP-47 (e.g. `en-US`); the spec expects an ISO-639-1 code.
    const language = results[0]?.languageCode?.split('-')[0];
